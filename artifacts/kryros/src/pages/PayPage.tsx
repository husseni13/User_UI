...
import {
  ...
  ChevronRight, ChevronDown, Download, Headphones
} from "lucide-react";

type Screen = "main" | "card" | "mobile" | "bank" | "whatsapp" | "apple" | "google" | "success";
...
const METHODS = [
  { id: "card", label: "Card Payment", sub: "Visa, Mastercard & more", icon: CreditCard,
    logos: <div className="flex items-center gap-1">
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-5 pb-28 min-h-screen">
      ...
      <div className="flex items-center justify-between mb-1">
        <h1 className="text-2xl font-black text-foreground">Pay</h1>
        <div className="flex items-center gap-1.5 text-xs text-primary font-semibold">
          <Lock className="w-3.5 h-3.5" /> Secure Payment
        </div>
      </div>
      <p className="text-xs text-muted-foreground mb-5">Send money securely to KRYROS</p>
      ...
      <div className="bg-card border border-border rounded-2xl p-4 mb-3">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex items-center gap-1.5 text-xs font-semibold text-muted-foreground border border-border rounded-xl px-2.5 py-1.5">
            <div className="w-4 h-3 rounded-sm bg-red-500 flex-shrink-0" style={{ backgroundImage: "linear-gradient(#B22234 0%, #B22234 33%, #fff 33%, #fff 66%, #3C3B6E 66%)" }} />
        ...
        <p className="text-xs text-primary font-semibold">+ Available balance: $1,250.00</p>
      </div>

      {/* Payment Purpose */}
      <div className="bg-card border border-border rounded-2xl p-4 mb-3">
        <p className="text-xs font-semibold text-muted-foreground mb-1.5">Payment Purpose (Optional)</p>
        <div className="flex items-center justify-between border border-border rounded-xl px-3 py-2.5">
          <span className="text-sm text-foreground">Product Payment</span>
      ...
      <div className="bg-card border border-border rounded-2xl p-4 mb-3">
        ...
        <div className="space-y-2">
          {METHODS.map((m) => {
            ...
            return (
              <button key={m.id} onClick={() => !isComing && setMethod(m.id)} disabled={isComing}
                className={`w-full flex items-center gap-3 p-3 rounded-xl border-2 transition-all text-left ${isSelected ? "border-primary bg-primary/5" : isComing ? "border-border opacity-60" : "border-border hover:border-primary/30"}`}>
                ...
                {m.logos
                  ? <div className="flex-shrink-0">{m.logos}</div>
                  : !isComing && <ChevronRight className="w-3.5 h-3.5 text-muted-foreground flex-shrink-0" />
                }
              </button>
            );
          })}
      ...
      {method === "card" && (
        <div className="bg-card border border-border rounded-2xl p-4 mb-3 space-y-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 border border-border rounded-xl px-3 py-2.5 focus-within:ring-2 focus-within:ring-primary/30">
        ...
        <div className="bg-card border border-border rounded-2xl p-4 mb-3 space-y-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 border border-border rounded-xl px-3 py-2.5">
              ...
              <select className="flex-1 text-sm text-foreground outline-none bg-transparent">
                <option>Zambia (+260)</option><option>Ghana (+233)</option><option>Kenya (+254)</option>
              </select>
              <ChevronDown className="w-4 h-4 text-muted-foreground" />
            </div>
          </div>
          <div>
            ...
            <div className="flex items-center gap-2 border border-border rounded-xl px-3 py-2.5">
              <div className="bg-yellow-400 text-black text-[9px] font-black px-1.5 py-0.5 rounded">MTN</div>
              <select className="flex-1 text-sm text-foreground outline-none bg-transparent">
                <option>MTN Mobile Money</option><option>Airtel Money</option><option>Zamtel Money</option>
              </select>
              <ChevronDown className="w-4 h-4 text-muted-foreground" />
            </div>
          </div>
      ...
      {method === "whatsapp" && (
        <div className="bg-card border border-border rounded-2xl p-4 mb-3">
          ...
          <p className="text-xs text-muted-foreground mb-3">We will send your payment details to WhatsApp</p>
          <div className="bg-muted/40 rounded-xl p-3 space-y-1.5 text-xs mb-3">
      ...
      {(method === "apple" || method === "google") && (
        <div className="bg-card border border-border rounded-2xl p-4 mb-3 space-y-3">
          <button className="w-full py-3.5 rounded-xl font-bold text-sm flex items-center justify-center gap-2 mb-2"
            onClick={handlePay}
            style={{ background: method === "apple" ? "#000" : "#fff", color: method === "apple" ? "#fff" : "#000", border: method === "google" ? "1px solid #ddd" : "none" }}>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-background/95 backdrop-blur-xl border-t border-border px-4 py-3 z-30">
        <div className="max-w-lg mx-auto space-y-2">
          ...
          <p className="text-[10px] text-center text-muted-foreground">
            By continuing, you agree to our <Link href="/terms"><span className="text-primary underline cursor-pointer">Terms of Service</span></Link>
          </p>
        </div>
      </div>
    </div>
